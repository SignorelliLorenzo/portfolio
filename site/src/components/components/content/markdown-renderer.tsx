...
const resolveAssetSrc = (src: string | undefined, projectId?: string) => {
  ...
  // Rewrite legacy /api/assets/<projectId>/<file> to /projects/<projectId>/assets/<file>
  const legacyMatch = src.match(/^\/api\/assets\/([^/]+)\/(.+?)(\?.*)?$/);
  if (legacyMatch) {
    return `/projects/${legacyMatch[1]}/assets/${legacyMatch[2]}`;
  }
  // Already a /projects/... path — use as-is
  if (src.startsWith("/projects/")) return src;
  // Relative or bare filename — resolve via projectId
  if (projectId) {
...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({
  ...
  const markdownComponents = {
    ...
    p: ({ node, children, ...props }: any) => {
      const hasBlock = React.Children.toArray(children).some(
        (child: any) =>
          typeof child === "object" &&
          child?.type &&
          ["pre", "div", "img", "video", "table", "blockquote", "figure", "hr"].includes(child.type)
      );
      if (hasBlock) {
        return (
          <div
    ...
    pre: ({ node, ...props }: any) => (
      <pre
        className="relative overflow-x-auto rounded-3xl border border-border bg-card px-5 py-6 shadow-lg"
        {...props}
      />
    ),
    code: ({ node, className, ...props }: any) => {
      const isBlock = node?.position && node?.children?.[0]?.type === "text" && className;
      if (isBlock) {
        return (
  ...
  return (
    <section className="relative isolate max-w-6xl mx-auto px-4 sm:px-10">
      <article
        className={`${GeistSans.className} relative space-y-8 text-muted-foreground tracking-[0.005em] leading-[1.9] max-w-5xl md:max-w-6xl mx-auto rounded-3xl border border-border/40 bg-card/30 backdrop-blur-sm px-6 sm:px-12 py-12 shadow-xl`}
      >
        <ReactMarkdown
